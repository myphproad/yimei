	...
	public function moreList(){
		...
		if(!empty($list2)){
			foreach($list2 as $k=>$v){
				...
				$tpl .='<div style="clear:both"></div><div><h5>'.$v['title'].'</h5><div>'.htmlspecialchars_decode($list2["$k"]["content"],ENT_QUOTES).'</div></div><a href="'.U('comment',array('tid'=>$list2["$k"]["id"],'wecha_id'=>get_openid(),'token'=>$list2["$k"]["token"])).'">查看全文</a></section><footer><ul class="box"><li>';
				...
				$tpl .= '</li><li><a href="'.U('comment',array('tid'=>$v['id'],'wecha_id'=>get_openid(),'token'=>$token)).'" class="a_comment"><span class="icons icons_comment" >&nbsp;</span><label>'.$v['cnum'].'</label></a></li><li>';
