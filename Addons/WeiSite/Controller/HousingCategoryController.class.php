...
class HousingCategoryController extends BaseController {
	var $model;
	function _initialize() {
		$this->model = $this->getModel ( 'WeisiteHousingCategory' );
		parent::_initialize ();
	}
	// 通用插件的列表模型
...
// 		$map = $this->_search_map ( $this->model, $fields );
		...
		if (isset ( $_REQUEST [$key] ) && ! isset ( $map [$key] )) {
 		...
 		$list_data['list_data']=M('WeisiteHousingCategory')->where($map)->order('id desc')->select();
