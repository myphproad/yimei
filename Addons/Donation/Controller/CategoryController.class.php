...
// 		$map = $this->_search_map ( $this->model, $fields );
		...
		if (isset ( $_REQUEST [$key] ) && ! isset ( $map [$key] )) {
 		...
 		$list_data['list_data']=M('Donation_category')->where($map)->order('id desc')->select();
...
// 			$data['ids'].="<a target='_self' href=$durl>删除</a>&nbsp;";
	...
	public function edit($model = null, $id = 0) {
		...
		if (IS_POST) {
			...
			$tmpImg=ONETHINK_ADDON_PATH.'Donation/View/default/TemplateSubcate/'.$data['template'].'/icon.png';
