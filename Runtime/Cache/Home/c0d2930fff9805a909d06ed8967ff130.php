      ...
      <?php if($add_button || $del_button || $search_button): ?><div class="table-bar">
        <div class="fl">
          <?php if(empty($model["extend"])): ?><div class="tools">
				<?php if($add_button): $add_url || $add_url = U('add?model='._ACTION); ?><a class="btn" href="<?php echo ($add_url); ?>">新 增</a><?php endif; ?>
				<?php if($checkTopics_button): $checkTopics_url || $checkTopics_url = U('checkTopics?model='._ACTION); ?><button class="btn ajax-post confirm" target-form="ids" url="<?php echo ($checkTopics_url); ?>">审核</button><?php endif; ?>    
				<?php if($del_button): $del_url || $del_url = U('del?model='._ACTION); ?><button class="btn ajax-post confirm" target-form="ids" url="<?php echo ($del_url); ?>">删 除</button><?php endif; ?>    
